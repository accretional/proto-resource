...
// Suitable for public flows like device authorization where no server-side
// secret is needed.
...
// Init bootstraps a WorkOS client, then either verifies an access token
// or falls through to the invitation flow. Call after flag.Parse().
func Init() {
	if !flag.Parsed() {
		flag.Parse()
	}
