...
// LoadUser retrieves a UserState.  Returns nil, false when the key is absent.
...
// UserCount returns the current number of tracked users.
func UserCount() int {
	n := 0
...
// UpdateUserSessions fetches fresh sessions from the API and patches the
...
// UpdateUserIdentities fetches fresh identities from the API and patches the
// stored state in-place.
