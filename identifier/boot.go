...
var (
	...
	// tokens is a concurrent map[string]*pb.Resource.
	tokens sync.Map
...
func StoreToken(key string, r *pb.Resource) {
	tokens.Store(key, r)
}

// LoadToken retrieves a Resource from the token map.
func LoadToken(key string) (*pb.Resource, bool) {
	v, ok := tokens.Load(key)
	if !ok {
		return nil, false
	}
	return v.(*pb.Resource), true
}
